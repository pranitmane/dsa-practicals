...
void displayAll() {
    ...
    file.read((char*)&x, sizeof(x)); // Read the first record
    while (!file.eof()) { // Loop until end-of-file
        x.display();
        file.read((char*)&x, sizeof(x)); // Read the next record
    }
    file.close();
}
...
void insertRecord() {
    ...
    while (file.read((char*)&x, sizeof(x))) {
        if (x.getEmpCode() > newEmp.getEmpCode()) {
            ...
            break; // Added break to avoid writing 'x' twice
        }
        fin.write((char*)&x, sizeof(x));
    }
    // Handling the case where the new record has the highest code
    if (file.eof()) {
        fin.write((char*)&newEmp, sizeof(newEmp));
    }
    fin.close();
